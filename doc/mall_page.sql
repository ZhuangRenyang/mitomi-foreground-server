...
-- 相册表：为相册名称字段添加索引
create index idx_album_name on page_album (name);
...
-- 商品表：创建数据表
drop table if exists page_commodity;
create table page_commodity
(
    id                     bigint unsigned auto_increment comment '记录id',
    ...
    price                  double(10,2)unsigned default 0 comment '价格',
...
-- 商品表：为商品名称字段添加索引
create index idx_commodity_name on page_commodity (name);
...
-- 类别表：为类别名称字段添加索引
create index idx_category_name on page_category (name);

-- 品牌类别关联表：创建数据表
drop table if exists page_commodity_category;
create table page_commodity_category
(
    id           bigint unsigned auto_increment comment '记录id',
    commodity_id     bigint unsigned default null comment '商品id',
...
-- 属性模版表：创建数据表
drop table if exists page_attribute_template;
create table page_attribute_template
(
    id           bigint unsigned auto_increment comment '记录id',
...
-- 属性模版表：为属性模版名称字段添加索引
create index idx_attribute_template_name on page_attribute_template (name);

-- 类别与属性模版关联表：创建数据表
drop table if exists page_category_attribute_template;
create table page_category_attribute_template
(
    id                    bigint unsigned auto_increment comment '记录id',
    category_id           bigint unsigned default null comment '类别id',
...
drop table if exists page_spu;
create table page_spu
(
    ...
    commodity_id               bigint unsigned  default null comment '品牌id',
    commodity_name             varchar(50)      default null comment '品牌名称（冗余）',
    category_id            bigint unsigned  default null comment '类别id',
...
-- SPU详情表：创建数据表
drop table if exists page_spu_detail;
create table page_spu_detail
(
    id           bigint unsigned auto_increment comment '记录id',
...
drop table if exists page_sku;
create table page_sku
(
    id                     bigint unsigned not null comment '记录id',
    spu_id                 bigint unsigned  default null comment 'SPU id',
    title                  varchar(255)     default null comment '标题',
...
drop table if exists page_sku_specification;
create table page_sku_specification
(
    id              bigint unsigned auto_increment comment '记录id',
...
-- 商品表：插入测试数据
insert into page_commodity (name, category, description, keywords, enable)
values ('华为', '数码', '华为专注网络设备三十年', '华为,huawei,mate,magicbook', 1),
       ('小米', '数码', '小米，为发烧而生', '小米,xiaomi,发烧', 1),
       ('苹果', '数码', '苹果，全球知名品牌', '苹果,apple,pingguo,iphone,mac', 1);

-- 类别表：插入测试数据
insert into page_category (name, parent_id, depth, is_parent, keywords, enable, is_display)
...
-- 商品类别表：插入测试数据
insert into page_commodity_category (commodity_id, category_id)
values (1, 3),
       (2, 3),
       (3, 3),
       (1, 8),
...
-- 关联测试查询：各商品有哪些类别的产品
select page_commodity_category.id, page_commodity.name, page_category.name
from page_commodity_category
         left join page_commodity
                   on page_commodity_category.commodity_id = page_commodity.id
         left join page_category
                   on page_commodity_category.category_id = page_category.id
order by page_commodity.category;

-- 属性表：插入测试数据
insert into page_attribute (name, description, type, input_type, value_list, unit, is_allow_customize)
...
-- 属性模版表：插入测试数据
insert into page_attribute_template (name, pinyin, keywords)
...
-- 相册表：插入测试数据
insert into page_album (name, description)
values ('iPhone 13', null),
       ('Mi 11 Ultra', null);

-- 图片表：插入测试数据
insert into page_picture (album_id, url, description, width, height)
...
-- SPU表：插入测试数据
insert into page_spu (id, name, type_number, title, description, list_price, stock, stock_threshold, unit, commodity_id,
                      commodity_name, category_id, category_name, keywords, tags)
...
-- SPU详情表：插入测试数据
insert into page_spu_detail (spu_id, detail)
values (1, '<div>iPhone 13的详情HTML</div>'),
       (2, '<div>小米11 Ultra的详情HTML</div>');

-- SKU（Stock Keeping Unit）表：插入测试数据
insert into page_sku (id, spu_id, title, attribute_template_id, specifications, price, stock, stock_threshold)
...
insert into page_sku_specification (sku_id, attribute_id, attribute_name, attribute_value, unit)
...
desc page_category;
desc page_commodity;
desc page_commodity_category;
desc page_attribute;
desc page_attribute_template;
desc page_spu;
desc page_spu_detail;
desc page_sku;
desc page_sku_specification;